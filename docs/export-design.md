...
`export_service.export_events()` calls `app/repositories/audit_event_repository.py::list_events_for_export()` — a new function, deliberately **not** `list_events()` (the paginated query API, which excludes archived records by design - see `docs/redaction-design.md`'s sibling document, `session.md` §8) and **not** `list_all_events()` (used by verification, which takes no filter at all). Export needs both: filtered, and archive-inclusive. `list_events_for_export()` filters only on `actorId`/`resourceId`, never on `archived_at`, so a record that retention has archived is still exported - retention changes what's *visible in routine queries*, not what's *exportable history* for a specific actor or resource. `archivedAt` is included per record (via the reused `AuditEventOut` schema) so a recipient can see which records were archived at export time.
